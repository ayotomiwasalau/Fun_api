...
##Post joke test
	...
	def test_post_joke(self):
		res = self.client().post('/jokes', json={'title':'laugh', 'joke':'two men walked into the bar, one named Peter, one named Paul'})
		data = json.loads(res.data)
...
# ##Update joke
	...
	def test_updating_joke(self):
		res = self.client().patch('/jokes/30', json={'joke':'If i am down, what can I use to cheer myself up? Pull up'})
		data = json.loads(res.data)
...
# ##Deleting jokes***
	...
	def test_deleting_joke(self):
		res = self.client().delete('/jokes/35')
		data = json.loads(res.data)
...
##Post answer to riddle
	...
	def test_post_answer_to_riddle(self):
		res = self.client().post('/riddle/4/answer', json={'answer': 'fingers'})
		data = json.loads(res.data)
...
##Post riddle test
	...
	def test_post_riddle(self):
		res = self.client().post('/riddle', json={'riddles':'what do they call cash liquid?', 'answer':'it makes you float', })
		data = json.loads(res.data)
...
##Update riddle
	...
	def test_updating_riddle(self):
		res = self.client().patch('/riddle/23', json={'riddle':'what do they call cash liquid?', 'answer': 'Because it keeps you rest a-shored'})
		data = json.loads(res.data)
...
##Deleting riddle
	...
	def test_deleting_riddle(self):
		res = self.client().delete('/riddle/34')
		data = json.loads(res.data)
...
##Post proverb test
	...
	def test_post_proverb(self):
		res = self.client().post('/proverbs', json={'proverb':'The goat that refuses to flee when he see a boil pot of water, should be ready to take a hot bath'})
		data = json.loads(res.data)
...
#**
	def test_updating_proverb(self):
		res = self.client().patch('/proverbs/22', json={'proverb': 'the apple that falls all the branch shows he is ripe'})
		data = json.loads(res.data)
...
#**
	def test_deleting_proverb(self):
		res = self.client().delete('/proverbs/28')
		data = json.loads(res.data)
